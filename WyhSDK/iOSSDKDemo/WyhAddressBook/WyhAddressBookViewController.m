//
//  WyhAddressBookViewController.m
//  WyhSDK
//
...
@interface WyhAddressBookViewController ()<UITableViewDelegate,UITableViewDataSource>
@property(nonatomic,strong)UITableView *table;
@property(nonatomic,strong)NSMutableArray *dataArray;
@end

@implementation WyhAddressBookViewController
-(NSMutableArray *)dataArray{
    if (!_dataArray) {
        _dataArray = [NSMutableArray new];
        WyhModel *model = WyhModel.new;
        model.type = ModelTypeString;
        model.textStr = @"AddressBook通讯录框架\niOS9 AddressBook,AddressBookUI不推荐使用了，用Contacts,ContactsUI代替。";
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.wyhTitle.wyhtext(@"AddressBook") ;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row==1) {
        ...
        [WyhAddressBook request];
    }else if (indexPath.row==2){
        [WyhAddressBook getPerson];
    }else if (indexPath.row==3){
        [WyhAddressBook newPerson];
    }
    if (vc) {
        [self wyhPush:vc];
    }else{
