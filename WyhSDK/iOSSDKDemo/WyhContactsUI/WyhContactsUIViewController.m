...
@interface WyhContactsUIViewController ()<UITableViewDelegate,UITableViewDataSource>
@property(nonatomic,strong)UITableView *table;
@property(nonatomic,strong)NSMutableArray *dataArray;
@end

@implementation WyhContactsUIViewController
-(NSMutableArray *)dataArray{
    if (!_dataArray) {
        _dataArray = [NSMutableArray new];
        WyhModel *model = WyhModel.new;
        model.type = ModelTypeString;
        model.textStr = @"ContactsUI通讯录框架\n提供了联系人列表界面/详情界面/添加联系人界面,一般用于查看联系人/添加联系人等功能";
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.wyhTitle.wyhtext(@"ContactsUI") ;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row==1) {
        ...
        [WyhContactsUI getContacts:self];
