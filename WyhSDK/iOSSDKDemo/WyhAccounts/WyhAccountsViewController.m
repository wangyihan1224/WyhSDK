...
@interface WyhAccountsViewController ()<UITableViewDelegate,UITableViewDataSource>
@property(nonatomic,strong)UITableView *table;
@property(nonatomic,strong)NSMutableArray *dataArray;
@end

@implementation WyhAccountsViewController
-(NSMutableArray *)dataArray{
    if (!_dataArray) {
        _dataArray = [NSMutableArray new];
        WyhModel *model = WyhModel.new;
        model.type = ModelTypeString;
        model.textStr = @"Accounts框架\n帮助用户访问和管理他们的外部帐户，而不需要他们进入账户的登录认证。(已弃用)";
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.wyhTitle.wyhtext(@"Accounts") ;
