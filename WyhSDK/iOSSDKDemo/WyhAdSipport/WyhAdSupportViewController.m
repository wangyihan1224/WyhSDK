...
@interface WyhAdSupportViewController ()<UITableViewDelegate,UITableViewDataSource>
@property(nonatomic,strong)UITableView *table;
@property(nonatomic,strong)NSMutableArray *dataArray;
@end

@implementation WyhAdSupportViewController
-(NSMutableArray *)dataArray{
    if (!_dataArray) {
        _dataArray = [NSMutableArray new];
        WyhModel *model = WyhModel.new;
        model.type = ModelTypeString;
        model.textStr = @"AdSupport广告支持框架\n提供访问用户的广告标示符号，以及判断用户是否限制了广告跟踪。";
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.wyhTitle.wyhtext(@"AdSupport") ;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row==1) {
        [WyhAdSupport getUUID];
    }else if (indexPath.row==2){
        [WyhAdSupport lookIsAdEnabled];
